...
if [ -z ${STATIONEERS_SETTINGS_PATH+x} ]; then
	STATIONEERS_STARTUP_COMMAND="${STATIONEERS_STARTUP_COMMAND} -settings GamePort ${STATIONEERS_GAME_PORT} ServerName \"${STATIONEERS_SERVER_NAME}\" ServerPassword \"${STATIONEERS_SERVER_PASSWORD}\" AutoSave true SaveInterval ${STATIONEERS_SERVER_SAVE_INTERVAL} ServerAuthSecret \"${STATIONEERS_SERVER_SECRET}\""
fi
